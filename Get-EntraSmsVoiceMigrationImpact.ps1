...
function New-HtmlReport {
    ...
    # Excluded rows are left out, because PasswordlessCapableInScope -- the numerator this
    # becomes the denominator for -- is counted over candidate rows, which never include
    # them. With fifty in-scope service accounts regexed out, the progress percentage was
    # quietly depressed by users the operator had asked the report to ignore.
    $inScopeCount = @($Rows | Where-Object { ($_.InSmsPolicyScope -or $_.InVoicePolicyScope) -and $_.Risk -ne 'Excluded' }).Count
...
function New-ActionList {
    ...
    # Band membership, or the lockout flag. Today those pick the same people, because a
    # user with nothing surviving cannot land in Low; the guard makes the headline count
    # and this queue provably the same set rather than coincidentally so.
    $entries = foreach ($row in @($Rows | Where-Object {
                $_.Risk -in @('Critical', 'High', 'Moderate') -or (Test-RowFlag (Get-PropertyValue $_ 'BlockedAtRetirement'))
            })) {
...
function New-TicketExport {
    ...
    # Stranded first, then privileged, then everyone else. When -MaxIndividualTickets cuts
    # this list, this order decides who falls into the bulk ticket -- and it used to be
    # admin-then-alphabetical, so a phone-only "Zoe" actually stopped at sign-in went to
    # the bulk pile while a non-blocked "Aaron" with Authenticator got his own P2. The
    # action list ranks lockout above admin; the tickets now agree with it.
    $highRows = @($Rows | Where-Object Risk -eq 'High' | Where-Object { & $needsTicket $_ } |
        Sort-Object -Property @{ Expression = { Test-RowFlag $_.BlockedAtRetirement }; Descending = $true },
            @{ Expression = 'IsAdmin'; Descending = $true }, DisplayName)
...
"@
    ...
    if ($batchedHigh.Count -gt 0) {
        ...
        # Anyone stranded who overflowed into this ticket is named at the top of it, so a
        # lockout does not disappear into "run a campaign" because the tenant was large.
        $batchedBlocked = @($batchedHigh | Where-Object { Test-RowFlag $_.BlockedAtRetirement })
        $blockedLine = if ($batchedBlocked.Count -gt 0) {
            "Of these, $($batchedBlocked.Count) hold a phone as their ONLY method that satisfies MFA and are stopped at sign-in on 2027-02-01. Work those first: $(($batchedBlocked | Select-Object -First 10 | ForEach-Object { $_.UserPrincipalName }) -join ', ')$(if ($batchedBlocked.Count -gt 10) { ', ...' })"
        } else { 'None of these is stopped at sign-in on 2027-02-01: each holds a method that survives, so this is migration work rather than rescue.' }
        $campaignLine = if ($today -lt $autoEnable) {
            'rather than waiting for the Microsoft-managed default to fire on 2026-09-01.'
        } else {
            'now that the Microsoft-managed default has been in effect since 2026-09-01.'
        }
        $body = @"
...
3. Run a registration campaign scoped to that group $campaignLine
...
$summary = [PSCustomObject][ordered]@{
    ...
    # State on every entry. Without it a disabled or report-only policy on a retiring-only
    # strength read as a live lockout path -- the same mistake the green MFA line below was
    # already fixed for, one line up from it.
    CaPoliciesOnSmsVoiceStrength = if (-not $caReadable) { '(unreadable)' } else {
        (@($caOnRetiringStrength | ForEach-Object { "$($_.DisplayName) [$($_.State)] via strength '$($_.AuthStrengthName)'" }) -join ' | ')
    }
...
if (-not $caReadable) {
    ...
    if ($caOnRetiringStrength.Count -gt 0) {
        $caStrengthEnabled = @($caOnRetiringStrength | Where-Object State -eq 'enabled')
        $caStrengthInert = @($caOnRetiringStrength | Where-Object State -ne 'enabled')
        if ($caStrengthEnabled.Count -gt 0) {
            $names = (@($caStrengthEnabled | ForEach-Object { "$($_.DisplayName) via strength '$($_.AuthStrengthName)'" }) -join ' | ')
            Write-Host "  Conditional Access: $($caStrengthEnabled.Count) enabled polic$(if ($caStrengthEnabled.Count -eq 1) { 'y grants' } else { 'ies grant' }) through an SMS/voice-permitting strength: $names." -ForegroundColor Yellow
        }
        if ($caStrengthInert.Count -gt 0) {
            $names = (@($caStrengthInert | ForEach-Object { "$($_.DisplayName) [$($_.State)] via strength '$($_.AuthStrengthName)'" }) -join ' | ')
            Write-Host "    $($caStrengthInert.Count) more on such a strength $(if ($caStrengthInert.Count -eq 1) { 'is' } else { 'are' }) disabled or report-only and enforce nothing today: $names." -ForegroundColor Cyan
        }
    }
