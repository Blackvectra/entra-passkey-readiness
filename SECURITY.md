...
| Content Security Policy on the report | `<meta http-equiv="Content-Security-Policy">` | Defense in depth. Even with an encoding bug, nothing can execute or make an outbound request. `referrer` is also suppressed. |
| Output permission restriction | `Protect-OutputFile`, on by default on every platform | A new file otherwise takes its permissions from where it lands. On Windows that means inheriting the parent directory ACL, which on a shared reports folder can mean everyone reads your list of weak privileged accounts; on Linux and macOS it means whatever the umask allows, which on most distributions leaves it world-readable. Windows files are restricted to the owner and local Administrators; Linux and macOS files are set to `0600`, directories to `0700`. Setting POSIX permissions needs PowerShell 7.3 or later — on 7.0 to 7.2 the run warns and prints the `chmod` to run rather than skipping silently. Disable with `-SkipAclHardening` only where the filesystem rejects permission changes. |
| Parallel handoff isolation | `Invoke-EntraSmsVoiceSweep.ps1` | A concurrent sweep hands each child process its parameters through a directory in the shared temp location. It holds no user rows, but it names every customer being assessed, where their evidence is being written, and which app registration is reading their tenant. The directory is restricted to the owner before anything is written into it, and removed when the sweep ends. |
| Path traversal guard | `Invoke-EntraSmsVoiceSweep.ps1` label sanitization | Tenant list CSVs are often generated from a PSA export, so labels are untrusted. Path characters are replaced, leading and trailing dots stripped, and only the leaf retained, so a label cannot write outside `-ReportRoot`. |
